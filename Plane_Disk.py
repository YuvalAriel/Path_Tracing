...
# n= plane normal
# (p-p0)dot(n)=0  # p-p0 is any vector on the plane. dot product of two perpendicular vectors equals zero.
...
class Plane:
    ...
    def __init__(self, normal, point, material=Material([0, 1, 0], reflection=0.8)):
    ...
    def plane_intersect(self, ray):
        """ returns t if intersected. if not returns False. hit_point = origin +d*l"""
        n = self.plane_norm
...
class Disk:
    ...
    def disk_intersect(self, ray):
        ...
        # t = Plane.plane_intersect(Plane(self.disk_norm,),ray)
        n = self.disk_norm
